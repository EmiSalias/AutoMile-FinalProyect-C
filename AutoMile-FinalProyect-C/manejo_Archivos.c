...
void registrarAlquilerUsuario(int posArrDinVehi, int x, int y, int dni)
{
    ...
    if (arrDinVehiculos[posArrDinVehi].disponibilidad == 0) {
        ...
        MUTEXLOCK;
        gotoxy((medidasConsola.ancho / 2 - ANCHO / 2) + 6, (medidasConsola.alto / 2 - ALTO / 2) - 6);
        printf("El vehículo seleccionado no está disponible para alquilar.");
        MUTEXUNLOCK;
        Sleep(3000);
        return;
    }

    // Obtener las fechas de inicio y fin del alquiler
    MUTEXLOCK;
    Fecha fechaInicio = cargarFecha("Ingrese la fecha de inicio");
    MUTEXUNLOCK;
    cuadroMenuInstantaneo(x, y);
    MUTEXLOCK;
    Fecha fechaFin = cargarFecha("Ingrese la fecha de fin");
    MUTEXUNLOCK;

    // Verificar solapamiento de fechas con alquileres existentes
    if (alquilerSeSolapa(arrDinAlquileres, cantAlquileres, arrDinVehiculos[posArrDinVehi].patente, fechaInicio, fechaFin)) {
        cuadroMenuInstantaneo(x, y);
        MUTEXLOCK;
        gotoxy((medidasConsola.ancho / 2 - ANCHO / 2) + 6, (medidasConsola.alto / 2 - ALTO / 2) - 6);
        printf("El alquiler se solapa con otro alquiler existente.");
        MUTEXUNLOCK;
        Sleep(3000);
        return;
    }

    // Proceder con el registro del alquiler
    FILE *archi = fopen(ARCHI_ALQUILERES, "ab");
    if (!archi) {
        cuadroMenuInstantaneo(x, y);
        MUTEXLOCK;
    ...
    Alquiler nuevoAlquiler = ingresoAlquilerUsuario(arrDinVehiculos[posArrDinVehi].patente, arrDinVehiculos[posArrDinVehi].precioDeAlquilerDiario, dni, fechaInicio, fechaFin);

    // Cambiar la disponibilidad del vehículo
    arrDinVehiculos[posArrDinVehi].disponibilidad = 0;
    int posArchVehi = buscarPatenteArch(arrDinVehiculos[posArrDinVehi].patente);

    archi2 = fopen(ARCHI_VEHICULOS, "r+b");
    fseek(archi2, posArchVehi * sizeof(Vehiculo), SEEK_SET);
    fwrite(&arrDinVehiculos[posArrDinVehi], sizeof(Vehiculo), 1, archi2);

    cantAlquileres++;

    // Agregar el nuevo alquiler al arreglo dinámico
    if (cantAlquileres % 5 == 0) {
        arrDinAlquileres = (Alquiler *)realloc(arrDinAlquileres, (cantAlquileres + 5) * sizeof(Alquiler));
        if (!arrDinAlquileres) {
            perror("Error al realocar memoria para alquileres");
